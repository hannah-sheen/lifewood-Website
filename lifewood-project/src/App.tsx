...
function SiteLayout({ children, onShowCookieBanner }: any) {
  const location = useLocation();
  const isAdminLogin = location.pathname === ADMIN_LOGIN_PATH;
  const isDashboard = location.pathname === '/dashboard';
  const publicPaths = ['/', '/about', '/solutions', '/careers', '/contact', '/services', '/privacy-policy', '/cookie-policy', '/terms-and-conditions'];
  const isNotFound = !publicPaths.includes(location.pathname) && location.pathname !== ADMIN_LOGIN_PATH && location.pathname !== '/dashboard';
  const showNavFooter = !isAdminLogin && !isDashboard && !isNotFound;
...
export default function App() {
  ...
  const [hasEntered, setHasEntered] = useState(() => !!sessionStorage.getItem('splashDone'));
  const [splashMounted, setSplashMounted] = useState(() => !sessionStorage.getItem('splashDone'));
  const [cookieVisible, setCookieVisible] = useState(false);
  const playVideoRef = useRef<(() => void) | null>(null);

  const handleEnter = () => {
    setIsTransitioning(true);
    // Slices finish at ~1.35s (0.75 + 8*0.07), unmount splash and unlock content together
    setTimeout(() => {
      sessionStorage.setItem('splashDone', '1');
      setHasEntered(true);
    ...
    <BrowserRouter>
      ...
      {splashMounted && (
        <div className="fixed inset-0 z-[500]">
          <SplashScreen onEnter={handleEnter} isTransitioning={isTransitioning} />
        </div>
      )}

      {/* 2. THE CONTENT LAYER */}
      <div className={`relative ${!hasEntered ? 'pointer-events-none overflow-hidden h-screen' : ''}`}>
        <SiteLayout onShowCookieBanner={() => setCookieVisible(true)}>
          <Routes>
            <Route path="/" element={<Home playVideoRef={playVideoRef} />} />
            <Route path="/about" element={<About />} />
            <Route path="/solutions" element={<Solutions />} />
            ...
            <Route path="/cookie-policy" element={<CookiePolicy />} />
            <Route path="/terms-and-conditions" element={<TermsAndConditions />} />
            <Route path={ADMIN_LOGIN_PATH} element={<AdminRoute><LoginPage /></AdminRoute>} />
