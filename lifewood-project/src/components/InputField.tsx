...
//   const baseClass = `w-full bg-white rounded-2xl border border-transparent focus:ring-2 focus:ring-saffaron/50 outline-none transition-all text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed ${icon ? 'pl-11 pr-4 py-3.5' : 'px-4 py-3.5'} ${error ? 'ring-2 ring-red-400/50' : ''} ${className}`;
...
export default function InputField({ label, icon, error, className = '', required, as = 'input', ...props }: InputFieldProps) {
  // Border logic: default neutral gray, switch to saffaron on focus, red on error
  const borderState = error 
    ? 'border-red-400/50 focus:ring-red-400/50' 
    : 'border-darkSerpent/10 focus:border-transparent focus:ring-saffaron/50';

  const baseClass = `w-full bg-white rounded-2xl border ${borderState} outline-none transition-all text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed focus:ring-2 ${icon ? 'pl-11 pr-4 py-3.5' : 'px-4 py-3.5'} ${className}`;
